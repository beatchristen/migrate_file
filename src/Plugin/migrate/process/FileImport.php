...
class FileImport extends FileCopy {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    if ($this->getOverwriteMode() == \Drupal\Core\File\FileSystemInterface::EXISTS_ERROR && $this->isLocalUri($destination) && is_file($destination)) {
  ...
  protected function getOverwriteMode() {
    if (!empty($this->configuration['rename'])) {
      return \Drupal\Core\File\FileSystemInterface::EXISTS_RENAME;
    }
    if (!empty($this->configuration['reuse'])) {
      return \Drupal\Core\File\FileSystemInterface::EXISTS_ERROR;
    }

    return \Drupal\Core\File\FileSystemInterface::EXISTS_REPLACE;
  }
